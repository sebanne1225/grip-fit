...
0.1.0 初回リリース完了（2026-07-08）。VPM listing 掲載済み（VCC 導入可能）。GitHub Release 0.1.0 作成・asset 添付・listing build success まで完遂。default branch は main（初回リリース時に master から統一）。残: BOOTH 公開・VCC 実機確認（せ手作業・リリース管理 DB 追跡）。MVP 実装・実機検証は前スレ（2026-07-08_grip-fit-mvp-implementation）完了。
...
- 0.1.0 リリース済み（VPM 公開）。残: BOOTH 公開・VCC 実機（せ手作業・リリース管理 DB 追跡）。後続機能候補 = TD-GF-T2/T3（knowledge-base next-phase/tool-dev.md「## Grip Fit」節）
...
- `Runtime/GripFitOffset.cs` — データコンポーネント（offsetPosition/offsetRotation/hasRecordedValue）。`OnValidate` で `ModularAvatarBoneProxy` が同居していなければ自身を自動除去（誤操作防止、`nadena.dev.modular-avatar.core` を Runtime asmdef に参照追加済み）
